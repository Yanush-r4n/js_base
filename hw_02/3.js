...
let dayNumber = Number(
    prompt("Введи число в пределах от одного до тридцати одного")
);

if (dayNumber < 1 || dayNumber > 31 || dayNumber % 1 !== 0) {
    console.log("Неверное значение");
} else {
    let monthDecade = dayNumber;
    if (dayNumber % 10 === 0 || dayNumber === 31) {
        monthDecade = Math.trunc(dayNumber / 10);
    } else {
        monthDecade = Math.trunc(dayNumber / 10) + 1;
    }
    console.log(`Число ${dayNumber} попадает в ${monthDecade} декаду месяца.`);
}

/* 
Почему 4 % 10 = 4? В четверку ведь ни одна десятка не вмещается.
Да и если 0,0000004 % 10 = 4. Почему? А 0.0000004 % 10 = 0.0000004.
Кстати, а что быстрее выполняется? Математические операции или сравнения, так
как эту задачу можно решить не одним способом так точно.  
*/